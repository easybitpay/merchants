...
const closeOffcanvas = () => {
  const myOffcanvas = document.getElementById('emailVerify_offcanvas')
...
const verifyEmail = async () => {
  if (otpInputValue.value.length == 6) {
    ...
    // Request
    await store.verifyEmail(otpInputValue.value).then(res => {
        if (res) {
            closeOffcanvas()
        }
    })
...
<template>
  <div
    ...
    tabindex="-1"
    id="emailVerify_offcanvas"
    aria-labelledby="offcanvasEmailVerify"
  >
        ...
        <div class="card border-0 mb-4 min-h-354px">
          <div class="card-body pb-lg-22">
            ...
            <div class="d-flex gap-4">
              ...
              <div class="mb-10">
                <h3 class="mb-0 text-dark">Email Verify</h3>

                <p class="fs-7 mb-0 text-gray-800 dark-text-gray-600 ls-base">Email verify authentication</p>
              </div>
            </div>
            <!-- end::Header -->
